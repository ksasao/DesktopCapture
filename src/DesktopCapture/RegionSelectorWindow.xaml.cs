        ...
        private void Window_MouseUp(object sender, MouseButtonEventArgs e)
        {
            if (_isSelecting)
            {
                ...
                if (wpfWidth > 10 && wpfHeight > 10)
                {
                    ...
                    // WPF座標をスクリーン座標（物理ピクセル）に変換
                    var screenStartPoint = PointToScreen(new System.Windows.Point(wpfX, wpfY));
                    
                    // 物理ピクセルでの座標とサイズ
                    int x = (int)Math.Round(screenStartPoint.X);
                    int y = (int)Math.Round(screenStartPoint.Y);
                    int width = (int)Math.Round(wpfWidth * dpiScaleX);
                    int height = (int)Math.Round(wpfHeight * dpiScaleY);
